...
def object_count(frame, mf_size=3, thres=0.5, cl_size=2.6, op_size=5.2,
                 lbl_struct=np.ones((3, 3)), mfiltered=None, binarized=None,
                 closed=None, opened=None, labeled=None):
...
def beam_centering(cam, xmotor, zmotor, tolerance=5.0, max_iterations=10,
    ...
    debug:
        ...
        motor_coordinate        = motorposition at that step
                                  ndarray([xmot,zmot])
    ...
    def to_db(db, key, val, set_method='='):
        """Saves additional information for debugging purpose."""
    ...
    def get_data():
        ...
        db[-1]['deviation_from_border'] = \
            np.array([bc, np.array(frm.shape)-bc]).min()
        ...
        gv['dist'] = dist
        to_db(db, 'deviation_from_center',
              {'x': diff[0], 'y': diff[1], 'dist': dist})
    ...
    def move_motors(xpos=None, zpos=None, xmove=None, zmove=None,
        ...
        if np.equal([xpos, zpos, xmove, zmove], None).all():
            ...
            if not xmove is None:
                print 'xmove to', xmove
                xm = xmotor.move(xmove*mfact[0])
                if zmove is None:
                    xm.wait()
            if not zmove is None:
                print 'zmove to', zmove
                zmotor.move(zmove*mfact[1]).wait()

        db[-1]['motor_coordinate'] = \
            np.array([xmotor.position, zmotor.position])
        get_data()
        if gv['step'] == 100:
            exit()      # TODO remove after debug
    ...
    mfact = [cam.sensor_pixel_height, cam.sensor_pixel_width]
    gv = {'step': -1, 'iter': -1, 'fa_iter': 0, 'move_axes': [],
          'inv_axes': [], 'fshape': [], 'diff': [], 'dist': 0, 'bv': False}
    db = [] if debug is None else debug

    next_step('initial state', True)
    db[-1]['motor_coordinate'] = np.array([xmotor.position, zmotor.position])
    set_axes([[1., 0.], [0., 1.]] if move_axes is None else move_axes)
...
def test_beam_centering(testcase=0, tolerance=None, max_iterations=None,
    ...
    def _info(ndArr, bc, is_ax, try_ax):
        ...
        line(ndArr, [ndArr.shape[0]/2, 0],
             [ndArr.shape[0]/2, ndArr.shape[1]-1], (127*256+127)*256+127,
             '=', [3, 3])
        line(ndArr, [0, ndArr.shape[1]/2],
             [ndArr.shape[0]-1, ndArr.shape[1]/2], (127*256+127)*256+127,
             '=', [3, 3])
        ...
        col = [250*256*256, 250*256, (180*256+10)*256+120,
               (120*256+200)*256+10]
        ls = [None, None, [6, 3], [6, 3]]
        if not try_ax is None and not isinstance(try_ax, str):
            is_ax.extend(try_ax)
    ...
    def vis_debug(debug):
        ...
        for step in debug:
            ...
            if not isinstance(step['frame'], np.ndarray):
                print 'frame is not a np.ndarray (type=' + \
                    str(type(step['frame'])) + ')'
                frm = np.zeros(frame_size)
            else:
                frame = step['frame']
                frame_size = frame.shape
                frm = colorize(frame, [255, 255, 255], True)
                frm_rgb = frm.view(np.dtype([('b', 'u1'), ('g', 'u1'),
                                             ('r', 'u1'), ('a', 'u1')]))
            ...
            if fi:
                ...
                debug[-1]['win'].append(debug[-1]['win'][0].
                                        add_image(tabs[-1]))
    ...
    if testcase < 0:
        pass
    if int(testcase) == 0:
        tc_str = 'Random beamsize(10...60sigma)/' + \
            'beamposition([0.1<=x<0.9]*framesize)'
        c = [(0.1+0.8*random.random()) * cam._imgsize[0] *
             cam.sensor_pixel_width,
             (0.1+0.8*random.random()) * cam._imgsize[1] *
             cam.sensor_pixel_height]
    ...
    if testcase >= 6:
        ...
        cam.from_image(img)
        #cam._coord = (np.random.random(2)-0.5)*cam._image.shape*\
        #    [cam.sensor_pixel_height,cam.sensor_pixel_width]
        cam._coord = [2353*cam.sensor_pixel_width,
                      -204*cam.sensor_pixel_height]
        xmot._axis = [1, 0]
        zmot._axis = [0, 1]
